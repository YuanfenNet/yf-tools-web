...
function generate() {
  ...
  let charset = ''
  if (upperLetterChecked.value) {
    charset += upperLetters
  }
  if (lowerLetterChecked.value) {
    charset += lowerLetters
  }
  if (numberChecked.value) {
    charset += numbers
  }
...
<template>
  <page header="随机密码生成" class="page-password">
    ...
    <el-row class="controls-row">
      ...
      <el-col :xs="{ span: 12 }" :sm="{ span: 5 }" :lg="{ span: 3 }">
        <div class="flex-wrapper ignore-checkbox">
          <el-tooltip content="0oO1l|" placement="top">
            <el-checkbox v-model="ignoreConfusableCharacters">
              忽略易混淆字符
            </el-checkbox>
          </el-tooltip>
        </div>
      ...
      <el-col :xs="{ span: 24 }" :sm="{ span: 9 }">
        <div class="flex-wrapper checkbox">
          <el-checkbox v-model="upperLetterChecked">
            大写字母
          </el-checkbox>
          <el-checkbox v-model="lowerLetterChecked">
            小写字母
          </el-checkbox>
          <el-checkbox v-model="numberChecked">
            数字
          </el-checkbox>
        </div>
      </el-col>
      <el-col :xs="{ span: 24 }" :sm="{ span: 6 }">
        <div class="flex-wrapper">
          <el-checkbox v-model="symbolChecked">
            符号
          </el-checkbox>
    ...
    <div class="btn-wrapper">
      <el-button type="primary" size="large" round @click="generate">
        重新生成
      </el-button>
      <el-button v-if="isSupported" type="primary" size="large" round @click="copy(password)">
        复制密码
      </el-button>
    </div>
  </page>
